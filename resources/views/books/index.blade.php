        ...
        <div class="col-md-6">
            <form action="{{ route('books.index') }}">
                ...
                <div class="input-group">
                    <input type="text" name="keyword" class="form-control" placeholder="Filter by title" value="{{ Request::get('keyword') }}">
                ...
                <tbody>
                    @foreach ($books as $book)
                        <tr>
                            <td>
                                @if ($book->cover)
                                    <img src="{{ asset(Storage::url($book->cover)) }}" alt="" width="96px">
                                @endif
                            </td>
                            <td>{{ $book->title }}</td>
                            <td>{{ $book->author }}</td>
                            ...
                            <td>
                                ...
                                <form action="{{ route('books.destroy', $book->id) }}" class="d-inline" method="POST" onsubmit="return confirm('Move category to Trash?')">
                                    @csrf
